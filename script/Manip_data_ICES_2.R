...
#Regroupement donnees brutes 

#-------------------------------------2006_2019---------------------------------
#data 2006-2019
ICES2006_2019<-read_excel("C:/Users/vmartin/Desktop/Stage/GIT/Rouget_MATO/data/Données de débarquements/OfficialNominalCatches 2006 2019/ICESCatchDataset2006-2019.xlsx", 
                          sheet = 2)
...
#-------------------------------------1950-2010---------------------------------
#data 1950-2010
ICES1950_2010 <- read_excel("C:/Users/vmartin/Desktop/Stage/GIT/Rouget_MATO/data/Données de débarquements/HistoricalLandings1950-2010/ICES_1950-2010.xls")
#filter + correction
...
ICES1903_1949<-read_excel("C:/Users/vmartin/Desktop/Stage/GIT/Rouget_MATO/data/Données de débarquements/ICES1903-49/1903-1949_Landings.xlsx")
...
#sum landings 
ICES1903_2019_MUR_TLWbyYearbyCountry<-ICES1903_2019_MUR%>%group_by(Year, Country)%>% #do the sum of landings each year 
  summarise(TLW=sum(TLW,na.rm=T))%>%
  ungroup() 

ICES1903_2019_MUR_TLWbyYearbyCountry
...
View(ICES1903_2019_MUR_newnamecountry)

library(base)

ICES1903_2019_MUR_newnamecountry <-ICES1903_2019_MUR_TLWbyYearbyCountry %>%
  mutate(name_country = ifelse(Country %in% c("BE","BEL", "Belgium") , "Belgique",  
                            ifelse (Country %in% c("JE", "Channel Is.- Guernsey", "Channel Is.- Jersey", "Channel Islands (ns)", "GG"), "îles de la Manche", 
                                    ifelse (Country %in% c("DEU", "Germany"), "Allemagne",
                                            ifelse (Country %in% "Denmark", "Danemark",
                                                    ifelse (Country %in% c("FR", "France"), "France", 
                                                            ifelse (Country %in% c("ES", "Spain"), "Espagne",
                                                                   ifelse (Country %in% c("GB", "GBE", "UK - Scotland", "UK - Eng+Wales+N.Irl.", "UK - England & Wales"), "Royaume-Uni", 
                                                                           ifelse (Country %in% c("IE", "Ireland"), "Irelande", 
                                                                                   ifelse (Country %in% "Isle of Man", "Île de Man", 
                                                                                           ifelse (Country %in% "Italy", "Italie", 
                                                                                                   ifelse (Country %in% c("Netherlands", "NL", "NLD"), "Pays-bas", 
                                                                                                           ifelse (Country %in% c("NO", "Norway"), "Norvège", 
                                                                                                                   ifelse ( Country %in% c("PT", "PRT", "Portugal"), "Portugal", 
                                                                                                                           ifelse (Country %in% "SE", "Suède", "Autres")))))))))))))))
                                                                                                                                   
table(ICES1903_2019_MUR_newnamecountry$name_country)
#

MUR_TLWbyYearbyCountries <- ggplot(ICES1903_2019_MUR_newnamecountry) +
  geom_line (aes(x= as.numeric(Year), y = TLW, group =name_country, color = name_country)) +
  labs(x = "Year", y = "TLW (tons)", 
...
ggplot(ICES1903_2019_MUR_newnamecountry, aes(x= as.numeric(Year), y = TLW, fill = name_country))  + 
  geom_bar(stat="identity", color="grey")+
  theme_bw()+
  labs(x = "Années", y = "Débarquements (tonnes)", 
       title = "Débarquements de rouget-barbet de roche par pays entre 1905 et 2019", 
       fill = "Pays :")
...
ggplot(ICES1903_2019_MUR_newnamecountry, aes(x= as.numeric(Year), y = TLW, fill = name_country))  + 
  ...
  theme_bw()+
  labs(x = "Années", y = "Débarquements (tonnes)", 
       title = "Débarquements de rouget-barbet de roche par pays entre 1905 et 1950", 
       fill = "Pays :") + 
  scale_x_continuous(limits=c(1900, 1950))

ggplot(ICES1903_2019_MUR_newnamecountry, aes(x= as.numeric(Year), y = TLW, fill = name_country))  + 
  geom_bar(stat="identity", color="grey")+
  theme_bw()+
  labs(x = "Années", y = "Débarquements (tonnes)", 
       title = "Débarquements de rouget-barbet de roche par pays entre 1950 et 2019", 
       fill = "Pays :") + 
...
ICES1903_2019_MUR_FR <- ICES1950_2010 %>% 
  filter(Country %in% c("FR","France"))
...
MUR_TLWbyYearFrance <- ggplot(ICES1903_2019_MUR_FR_TLWbyYear , aes(x=Year, y=TLW)) +
  geom_point()+
  labs(x = "Year", y = "TLW (tons)", 
       title = "TLW for Mullus surmuletus France by year from 1905 to 2019") 
...
MUR_TLWbyYearbyCountries #pas tres beau
...
MUR_TLWbyYearbyFAO_Area  #pas tres beau
...
#Commande trÃ¨s utile pour afficher les valeurs d'une variable 

#table(ICES1903_2019_MUR$FAO_Area)
#ICES1903_2019_MUR%>%filter(grepl("27.8|VIII",FAO_Area))%>%pull(FAO_Area)%>%table
# le + pas besoin de tout affcihÃ©, affiche juste celle qui nous interresse 
